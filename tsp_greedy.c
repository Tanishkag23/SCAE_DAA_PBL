// tsp_greedy: SCAE Smart City Algorithm Engine Unit 5 NP: TSP — Nearest Neighbor Heuristic (Greedy Approx) Scal...
#include "../scae.h"

// tsp_greedy: Nearest neighbor from city 0 — O(n^2)
AlgoResult tsp_greedy(int* adjMatrix, int V) {
    ...
    // Single city — already "at destination"
    if (V <= 1) {
 ...
 // Nearest neighbor greedy: sabse paas wala unvisited city select karo
    for (int step = 1; step < V; step++) {
        int nearest = -1;
        int min_dist = INT_MAX;
 // Scan all cities to find nearest unvisited
        for (int j = 0; j < V; j++) {
            steps++;
        ...
        if (nearest == -1) {
            // Koi reachable unvisited city nahi! Graph disconnected hai.
            valid = 0;
            break;
        }
    ...
    // Return to city 0 (close the tour)
    if (valid) {
        steps++;
        if (adjMatrix[current * V + 0] == 0) {
            valid = 0; // Return road nahi hai!
        } else {
