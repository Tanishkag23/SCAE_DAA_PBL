...
AlgoResult dijkstra(int* adjMatrix, int V, int src, int dst) {
    ...
    for (int i = 0; i < V; i++) {
        // INT_MAX/2 trick: overflow se bachne ke liye INT_MAX/2 use karte hain addition ke waqt
        dist[i] = INT_MAX / 2;
        visited[i] = 0;
        parent[i] = -1;
    }
    ...
    // O(V^2) scan approach use kar rahe hain. Graph agar sparse ho toh Min-Heap better hoti hai.
    // Note: Negative weights yahan fail ho sakte hain kyunki Dijkstra nodes ko ek baar visited mark karta hai.
    ...
    if (dist[dst] == INT_MAX / 2) {
        ...
        // Path reconstruction: parent array use karke dst se src tak jate hain, fir reverse karte hain
        int curr = dst;
