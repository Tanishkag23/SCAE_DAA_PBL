...
AlgoResult insertion_sort(int* arr, int n) {
 ...
 // Caller ka array safe rakhna hai — apni copy pe kaam karo
    ...
    int* a = r.output;
 // Insertion: sort: taash ke paaton ki tarah sort karo — ek ek card uthao aur sahi jagah daalo Pehla element al...
    ...
    for (int i = 1; i < n; i++) {
        ...
        int key = a[i];
        int j   = i - 1;
 // City: mein nai complaints aati hain continuously — insertion sort unhe realtime sort kar sakta hai Near...
        ...
        // While: loop ek baar aur check kiya aur fail hua — woh comparison bhi count hoti hai! (ya toh j < 0 hua, ...
        r.steps++;
