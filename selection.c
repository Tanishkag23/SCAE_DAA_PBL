...
AlgoResult selection_sort(int* arr, int n) {
    ...
    // Selection sort: unsorted part se minimum dhundho, sorted part ke end mein daalo
    // Ek ek karke minimum element chunte jao — collection sort ki tarah!
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        // Flash memory ya EEPROM jaise write-expensive storage pe selection sort better hai
        // Kyunki writes zyada costly hain reads se — aur yahan writes minimum hain!
    }

    clock_t end = clock();

    // ALWAYS O(n^2): chahe already sorted ho ya reverse sorted — hamesha n*(n-1)/2 comparisons!
    // Bubble sort early exit kar sakta hai — selection sort nahi kar sakta. Yahi fark hai!
