...
AlgoResult bellman_ford(int* adjMatrix, int V, int src, int dst) {
    ...
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
            ...
            if (weight != 0) { // Non-zero weight signifies an edge
                edges[E].u = i;
                edges[E].v = j;
                edges[E].w = weight;
                E++;
    ...
    // V-1 rounds relax karte hain: Isko use karte hain kyunki graph me longest simple path me at max V-1 edges hoti hain.
    // Bellman-Ford Dijkstra ki jagah tab use karna chahiye jab negative edge weights ho.
    for (int i = 0; i < V - 1; i++) {
        for (int j = 0; j < E; j++) {
            int u = edges[j].u;
            int v = edges[j].v;
    ...
    // Negative cycle detection: Agar V-th round me bhi koi edge relax hoti hai, matlab graph me negative cycle hai.
    int negativeCycle = 0;
    for (int j = 0; j < E; j++) {
        int u = edges[j].u;
        int v = edges[j].v;
