...
AlgoResult prim_mst(int* adjMatrix, int V) {
    ...
    for (int i = 0; i < V; i++) {
        key[i] = INT_MAX / 2; // Key array initialization
        inMST[i] = 0;
        parent[i] = -1;
    }
    ...
    // Ye O(V^2) efficiency hai normal graphs ke liye. O((V+E)logV) heap wala use karein toh dense mein alag baat hogi.
    // Prim vs Dijkstra: PrimMST total weight minimize karta hai, Dijkstra single source path!
    return r;
}
